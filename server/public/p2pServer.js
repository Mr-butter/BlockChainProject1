...
const { isValidAddress } = require("./transaction");
...
function connectToPeer(port) {
  ...
  ws.on("error", () => {
    console.log("피어연결실패 / connection failed");
    console.log(ws.readyState);
    // WebSocket.readyState 읽기 전용 속성은  WebSocket현재 상태를 반환

    switch (ws.readyState) {
      case 2:
        console.log("case2 : 웹소켓 서버를 새로 생성합니다.");
        initP2PServer(6001);
        connectToPeer(6001);
        break;
      case 3:
        console.log("case3 : 웹소켓 서버를 새로 생성합니다.");
        initP2PServer(6001);
        connectToPeer(6001);
        break;
...
function handleBlockChainResponse(message) {
  ...
  if (latestReceiveBlock.header.index > latesMyBlock.header.index) {
    ...
    // (전달받은 블록이 내 블록보다 2개이상 긴 상태인데 마지막 블록 한개만 받은 상황)
      ...
      // 전달받은 블록이 한개니까 replaceChain 하기 위해 블록체인 달라고 요청
